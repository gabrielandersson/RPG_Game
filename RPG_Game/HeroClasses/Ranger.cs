...
namespace RPG_Game
{
    public class Ranger : Hero
    {
        ...
        public Ranger(string name) : base(name)
        {
            ...
            Damage = 1 * (1 + (PrimaryAttribute.Dexterity / 100));
        ...
        public string EquipItem(string itemName)
        {
            try
            {
                ...
                if (requestedItem is Armor)
                {
                    ...
                    if ((requestedItem as Armor).Category == ArmorCat.Leather || (requestedItem as Armor).Category == ArmorCat.Mail)
                    {
                ...
                if (requestedItem is Weapon)
                {
                    ...
                    if ((requestedItem as Weapon).Category == WeaponCat.Bows)
                    {
                        if (EquippedItems.ContainsKey(requestedItem.Slot))
                        {
                            var backToInventory = EquippedItems[requestedItem.Slot];
