...
namespace RPG_Game
{
    public class Mage : Hero
    {
        public Mage(string name) : base(name)
        {
            ...
            Damage = 1 * (1 + (PrimaryAttribute.Intelligence / 100));
        ...
        public string EquipItem(string itemName)
        {
            try
            {
                ...
                if (requestedItem is Armor)
                {
                    ...
                    if ((requestedItem as Armor).Category == ArmorCat.Cloth)
                    {
                ...
                if (requestedItem is Weapon)
                {
                    ...
                    if ((requestedItem as Weapon).Category == WeaponCat.Staffs || (requestedItem as Weapon).Category == WeaponCat.Wands)
                    {
                        if (EquippedItems.ContainsKey(requestedItem.Slot))
                        {
                            var backToInventory = EquippedItems[requestedItem.Slot];
